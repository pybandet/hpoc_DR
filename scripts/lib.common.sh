...
function prism_check() {
  ...
  local    _sleep=${SLEEP}
  local     _test=0
  local CURL_HTTP_OPTS="--max-time 25 --header Content-Type:application/json --header Accept:application/json --output /dev/null --insecure --show-error --write-out %{http_code}"
