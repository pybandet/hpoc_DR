...
log "---------------------------------------------------------------------------------------------------------------------------------"
log "Adding ${MSSQL19_SourceVM}, and then Creating Sync Software Profile."
log "---------------------------------------------------------------------------------------------------------------------------------"
...
log "Getting DB Server ID"
  ...
  _era_sync_db_server_id=$(curl ${CURL_HTTP_OPTS} -u ${ERA_USER}:${ERA_PASSWORD} -X GET "https://${ERA_HOST}/era/v0.9/dbservers" --data '{}' | jq '.[] | select(.name == "MSSQL19-ProfileSource") | .id' | tr -d \")
...
log "---------------------------------------------------------------------------------------------------------------------------------"
log "Adding ${MSSQL19_SourceVM}, and then Creating User Software Profile."
log "---------------------------------------------------------------------------------------------------------------------------------"
