...
PC_DEV_VERSION='pc.2021.1'
PC_CURRENT_VERSION='pc.2020.11'
PC_STABLE_VERSION='pc.2020.9'
FILES_VERSION='3.8.0'
FILE_ANALYTICS_VERSION='3.0.0'
...
STORAGE_IMAGES='SelfServiceContainer'
STORAGE_ERA='SelfServiceContainer'
ATTEMPTS=40
SLEEP=60
...
CURL_POST_OPTS="${CURL_OPTS} --max-time 25 --header Content-Type:application/json --header Accept:application/json --output /dev/null"
...
MSSQL19_USER_SourceVM="MSSQL19-USER-Profile"
MSSQL19_SYNC_SourceVM="MSSQL19-SYNC-Profile"
...
ERA_HOST=${IPV4_PREFIX}.$((${OCTET[3]} + 6))
CITRIX_DDC_HOST=${IPV4_PREFIX}.$((${OCTET[3]} + 7))
...
case "${OCTET[0]}.${OCTET[1]}" in
  ...
  10.42 ) # HPOC us-west = PHX
    ...
    PC_DEV_URL='http://10.42.95.45/pc.2021.1.tar'
    PC_CURRENT_METAURL='http://10.42.95.45/pcdeploy-pc.2020.11.json'
    PC_CURRENT_URL='http://10.42.95.45/pc.2020.11.tar'
    PC_STABLE_METAURL='http://10.42.95.45/pcdeploy-pc.2020.9.json'
    PC_STABLE_URL='http://10.42.95.45/pc.2020.9.tar'
    FILES_METAURL='http://10.42.95.45/afs-3.8.0.json'
    FILES_URL='http://10.42.95.45/nutanix-afs-el7.3-release-afs-3.8.0-stable.qcow2'
    FILE_ANALYTICS_METAURL='http://10.42.95.45/nutanix-file_analytics-el7.7-release-3.0.0-metadata.json'
    FILE_ANALYTICS_URL='http://10.42.95.45/nutanix-file_analytics-el7.7-release-3.0.0.qcow2'
    JQ_REPOS=(\
         'http://10.42.95.45/jq-linux64.dms' \
         #'http://10.55.251.38/workshop_staging/jq-linux64.dms' \
         #'https://github.com/stedolan/jq/releases/download/jq-1.5/jq-linux64' \
   )
    SSHPASS_REPOS=(\
       'http://10.42.95.45/sshpass-1.06-2.el7.x86_64.rpm' \
       #'http://10.55.251.38/workshop_staging/sshpass-1.06-2.el7.x86_64.rpm' \
       #'http://mirror.centos.org/centos/7/extras/x86_64/Packages/sshpass-1.06-2.el7.x86_64.rpm' \
    )
    QCOW2_REPOS=(\
  ...
  10.136 ) # HPOC us-west = BLR
    PC_DEV_METAURL='http://10.136.239.13/workshop_staging/pcdeploy-pc.2021.1.json'
    PC_DEV_URL='http://10.136.239.13/workshop_staging/pc.2021.1.tar'
    PC_CURRENT_METAURL='http://10.136.239.13/workshop_staging/pcdeploy-pc.2020.11.json'
    PC_CURRENT_URL='http://10.136.239.13/workshop_staging/pc.2020.11.tar'
    PC_STABLE_METAURL='http://10.136.239.13/workshop_staging/pcdeploy-pc.2020.9.json'
    PC_STABLE_URL='http://10.136.239.13/workshop_staging/pc.2020.9.tar'
    FILES_METAURL='http://10.136.239.13/workshop_staging/afs-3.8.0.json'
    FILES_URL='http://10.136.239.13/workshop_staging/nutanix-afs-el7.3-release-afs-3.8.0-stable.qcow2'
    FILE_ANALYTICS_METAURL='http://10.136.239.13/workshop_staging/nutanix-file_analytics-el7.7-release-3.0.0-metadata.json'
