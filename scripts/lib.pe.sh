...
EOF
    ...
    _test=$(curl ${CURL_POST_OPTS} --user ${PRISM_ADMIN}:${PE_PASSWORD} \
      ...
      "https://${PE_HOST}:9440/api/nutanix/v3/prism_central")
    #log "_test=|${_test}|"
  fi
}
...
function pe_license_api() {
  ...
  if (( $? == 0 )) ; then
    ...
    _test=$(curl ${URL_HTTP_OPTS} --user ${PRISM_ADMIN}:${PE_PASSWORD} -X POST --data '{
      "username": "SE with $(basename ${0})",
      "companyName": "Nutanix",
      "jobTitle": "SE"
    ...
    _test=$(curl ${URL_HTTP_OPTS} --user ${PRISM_ADMIN}:${PE_PASSWORD} -X PUT --data '{
      "defaultNutanixEmail": null,
      "emailContactList": null,
      "enable": false,
      "enableDefaultNutanixEmail": false,
